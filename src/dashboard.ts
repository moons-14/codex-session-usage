...
function runtimeDir(): string {
  ...
  const dir = join(tmpdir(), `codex-session-usage-${uid}`);
...
export async function serveDashboard(
  ...
  const server = Bun.serve({
    ...
    fetch(request) {
      ...
      if (url.pathname === "/health") return Response.json({ ok: true, token });
      if (url.pathname === "/api/sessions") return Response.json(snapshot);
